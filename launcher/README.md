...
- **Setup tab** — port (1024–65535, default 3000) and three models, each one of sonnet / opus /
  haiku / fable: the game master that answers chat, the puzzle writer that invents new questions,
  and the web collector that searches the web for existing ones. Also
...
`CLAUDE_MODEL`, `GENERATOR_MODEL` and `COLLECTOR_MODEL`, and extends `PATH` with the usual CLI locations (`~/.local/bin`, Homebrew, nvm…)
